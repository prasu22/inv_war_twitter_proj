...
#             # query3 and 4
            if re.compile("coronavirus",re.IGNORECASE).search(message['tweet']):
                ...
                if re.compile('|'.join(list_of_country), re.IGNORECASE).search(country):
                    ...
                    if country in countries_with_code:
                        country_code = countries_with_code[country]
                        if len(list_of_words) > 0:
                            for words in list_of_words.split(" "):
                               if db['top_100_words_country_code'].count_documents({"word":words,"country_code":country_code})==0:
                                   db['top_100_words_country_code'].insert_one({'word':words,"country_code":country_code,'count':1})
                               else:
                                   db['top_100_words_country_code'].update_one({'word': words, "country_code": country_code},{'$inc':{'count':1}})

# =======================================================================================================================================
# #               #query5
#                 if ((re.compile('|'.join(prevention_list), re.IGNORECASE).search(message['tweet'] )) and re.compile('@who|#who',re.IGNORECASE).search(message['tweet'])):
#                     country = message['country']
#                     print(country)
...
#                             #         db['top_10_prevention_country_code'].insert_one({'word':word,"country_code":country_code,'count':1})
#                             #     else:
...
# =======================================================================================================================================
              #query6
                ...
                if (re.compile("donat.*|contribut.*|money", re.IGNORECASE).search(message['tweet']) and re.compile('[$¢£¤¥֏؋৲৳৻૱௹฿៛₹](\d[0-9,.]+)',re.IGNORECASE).search(message['tweet'])):
                    country = message['country']
                    if re.compile('|'.join(list_of_country), re.IGNORECASE).search(country):
                       country = re.compile('|'.join(list_of_country), re.IGNORECASE).search(country).group(0).title()
                       print(message['tweet'])
