...
for message in my_consumer:
    ...
    try:
        if 'RT @' not in message['tweet']:
            ...
            #     if db['overall_tweet_per_country'].count_documents({'_id':message['_id'],"country":values['country'],"month":values['month']})==0:
            #         # print("adsf",db)
            #         db['overall_tweet_per_country'].insert_one({'_id':message['_id'],'count':values['count'],'country':values['country'],'month':values['month']})
            #         # print("happys",db['overall_tweet_per_country'].find())
            #     else:
            #         db['overall_tweet_per_country'].update_one({'_id':message['_id'],"country":values['country'],"month":values['month']},{'$inc':{'count':1}})
            #
            # # query2
            ...
            #         db['overall_tweet_per_country_on_daily_basis'].insert_one({'_id':message['_id'],'count': daily_basis_tweet['count'], 'country': daily_basis_tweet['country'], 'date': str(daily_basis_tweet['date'])})
            #     else:
            #         db['overall_tweet_per_country_on_daily_basis'].update_one({'_id':message['_id'],"country":daily_basis_tweet['country'],"date":str(daily_basis_tweet['date'])},{'$inc':{'count':1}})
            ...
            if re.compile("coronavirus",re.IGNORECASE).search(message['tweet']):
                ...
                if re.compile('|'.join(list_of_country), re.IGNORECASE).search(country):
                    ...
                    if db['top_100_words'].count_documents({'country':country})>100:
                        total_document_to_remove = db['top_100_words'].count_documents(({'country':country}))-100
                        print(total_document_to_remove)
                        for row in db['top_100_words'].find().sort('count', pymongo.ASCENDING).limit(total_document_to_remove):
                            print(row['_id'])
                            db['top_100_words'].delete_one({"_id":row['_id']})
