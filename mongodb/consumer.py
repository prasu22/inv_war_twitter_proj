            ...
            if re.compile("coronavirus",re.IGNORECASE).search(message['tweet']):
                ...
                if re.compile('|'.join(list_of_country), re.IGNORECASE).search(country):
                    ...
                    if db['top_100_words'].count_documents({'country':country})>100:
                        ...
                        # print(total_document_to_remove,country)
                        for row in db['top_100_words'].find({"country":country}).sort('count', pymongo.ASCENDING).limit(total_document_to_remove):
                            # print(row)
                            # print(row['_id'],row['count'],type(row['_id']))
