import re
from comman_variables.variable_files import DEFAULT_AMOUNT, CURRENCY_MAPPING, DONATION_KEYWORDS, DEFAULT_CURRENCY_NAME, \
...
def get_donation_amount(message):
    ...
    if re.compile('[$¢£¤¥֏؋৲৳৻૱௹฿៛₹](\s?)(\d[ 0-9,.]+)').search(message[TWEET_KEY]):
        amount = re.compile('[$¢£¤¥֏؋৲৳৻૱௹฿៛₹](\s?)(\d[ 0-9,.]+)').search(message[TWEET_KEY]).group(0)
        value = float(re.sub(r'[^\d.]', '', amount))
        message[DONATION_AMOUNT_KEY] = value
        return message

    elif re.compile(r"(\d[ 0-9,.]+)(\s?)(\bUSD\b|\bINR\b)", re.IGNORECASE).search(message[TWEET_KEY]):
        amount = re.compile(r"(\d[ 0-9,.]+)(\s?)(\bUSD\b|\bINR\b)", re.IGNORECASE).search(message[TWEET_KEY]).group(0)
        value = float(re.sub(r'[^\d.]', '', amount))
        message[DONATION_AMOUNT_KEY] = value
        return message
