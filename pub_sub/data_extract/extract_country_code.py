...
def get_country_code(message):
    ...
    country = re.compile('|'.join(list_of_country), re.IGNORECASE).search(message['country'])

    if country:
        country = re.compile('|'.join(list_of_country), re.IGNORECASE).search(message['country']).group(0).title()
