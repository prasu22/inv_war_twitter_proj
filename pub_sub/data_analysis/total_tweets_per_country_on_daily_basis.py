...
def analysis_total_tweet_per_country(message,db):
    ...
    if country_data:
        ...
        if daily_basis_tweet is not None:
            ...
            if db['a_overall_tweet_per_country_on_daily_basis'].count_documents({"country": daily_basis_tweet['country'], "date": str(daily_basis_tweet['date'])}) == 0:
                db['a_overall_tweet_per_country_on_daily_basis'].insert_one({'count': daily_basis_tweet['count'], 'country': daily_basis_tweet['country'],'country_code':country_code, 'date': str(daily_basis_tweet['date'])})
            else:
